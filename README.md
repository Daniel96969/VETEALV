...
Te proporciono el código para cada archivo de la estructura modular sugerida. Este código aplica los principios SOLID (especialmente SRP y DIP) , implementa encriptación de contraseñas con bcrypt y usa consultas parametrizadas para evitar inyección SQL.
...
Esta clase aplica el Principio de Responsabilidad Única (SRP). Su única responsabilidad es gestionar la conexión y la desconexión con la base de datos.
...
Este modelo maneja la lógica de usuarios y aplica la encriptación de contraseñas.
...
class Usuario:
    ...
    def crear(username: str, password_plano: str, nombre: str, email: str):
        ...
        Usa consultas parametrizadas para evitar inyección SQL[cite: 15].
        """
        if not username or not password_plano or not nombre:
            print("Error: Username, contraseña y nombre son requeridos.") [cite: 17]
            return False
        ...
        try:
            ...
            print(f"Error al crear usuario: {e}") [cite: 18]
            return False
...
Modelo para la gestión de libros. Todas las consultas son seguras.
...
biblioteca/main.py
Este es el punto de entrada de la aplicación, que consume los modelos.
...
Este es el contenido que debes poner en el archivo README.md en la raíz de tu nuevo repositorio de GitHub.

Markdown

# Ejercicio 12: Refactorización Sistema de Biblioteca (Buenas Prácticas y Seguridad)

[cite_start]Este proyecto es una refactorización del "Ejercicio 11 (Sistema de Biblioteca)" [cite: 4][cite_start], enfocado en aplicar buenas prácticas de desarrollo de software, principios SOLID y medidas de seguridad esenciales.

[cite_start]El objetivo es transformar un código funcional en un código mantenible, escalable, legible y seguro[cite: 3, 25].

## 🚀 Mejoras Realizadas (Respecto al Original)

[cite_start]Este proyecto implementa mejoras clave en estructura y seguridad:

### [cite_start]1. Aplicación de Principios SOLID 

* [cite_start]**Principio de Responsabilidad Única (SRP)**: El código se ha modularizado.
...
* [cite_start]**Principio de Inversión de Dependencias (DIP)**[cite: 12]: Los módulos de alto nivel (como `main.py` y los modelos) no dependen directamente de `mysql.connector`, sino de la abstracción proporcionada por `conexion.py` (`obtener_conexion`).

### [cite_start]2. Mejoras de Seguridad [cite: 13]

* [cite_start]**Encriptación de Contraseñas (Hashing)** [cite: 23][cite_start]: Las contraseñas de los usuarios **nunca** se guardan en texto plano[cite: 14]. Se utiliza la biblioteca `bcrypt` para generar un *hash* seguro de la contraseña antes de almacenarla. Al iniciar sesión, se compara el *hash* de la contraseña ingresada con el almacenado.

* [cite_start]**Prevención de Inyección SQL**: Se han eliminado todas las consultas basadas en concatenación de cadenas. [cite_start]En su lugar, se utilizan **consultas parametrizadas** (pasando los valores como un segundo argumento a `cursor.execute()`)[cite: 15]. Esto asegura que la base de datos trate las entradas del usuario como datos, y no como código SQL ejecutable.

* [cite_start]**Control de Errores**[cite: 18]: Se utiliza manejo de excepciones (`try...except`) para capturar errores de la base de datos y evitar la fuga de información sensible.

### [cite_start]3. Código Modular y Escalable [cite: 25]

[cite_start]La estructura de carpetas sugerida permite que el proyecto crezca fácilmente[cite: 30]. [cite_start]Si se necesitara añadir una nueva entidad (ej. "Autores" o "Editoriales")[cite: 9], simplemente se crearía un nuevo archivo en `modelos/` sin necesidad de modificar el código existente.
...
(Debes subir tu propia captura de pantalla al repositorio y enlazarla aquí [cite: 29])

`[Imagen de la aplicación ejecutándose en la consola]`

---
3. 🏆 Código para Puntos Extra (Inyección SQL)
Para ganar los puntos extra, debes crear un programa separado que sea deliberadamente vulnerable y demostrar el ataque.


Crea estos dos archivos fuera de tu proyecto biblioteca/.

vulnerable_login.py
Este archivo contiene el programa vulnerable que se conecta a MySQL y usa concatenación de cadenas.
...
Este archivo importa la función vulnerable y la explota , documentando el proceso.
...
def demostrar_ataque():
    ...
    # ------------------------------------------
    # 1. Consulta Normal (Funcionando) [cite: 47]
    # ------------------------------------------
    ...
    # ------------------------------------------
    # 2. Inyección SQL Exitosa [cite: 48]
    # ------------------------------------------
    ...
    # ------------------------------------------
    # 3. Resultados del Ataque [cite: 49]
    # ------------------------------------------
...
¡Solo te queda configurar tu base de datos, poner tu contraseña en los archivos conexion.py y vulnerable_login.py, crear tu repositorio y subir los archivos!
